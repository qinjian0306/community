package org.chinamyheart.community.common.utils;

import org.chinamyheart.community.common.exceptions.BCException;
...
public class Utils {
	 ...
	 *
	 * */
	public static String randomIntWithChar(int length) {
		StringBuffer sb = new StringBuffer();
		Random random = new Random();
		//参数length，表示生成几位随机数
		for(int i = 0; i < length; i++) {
	...
	public static long getTimesAddMin (int min){
		Calendar cal = Calendar.getInstance();
		cal.add(Calendar.MINUTE, min);
		return cal.getTimeInMillis();
	}

	public static long getTimesAddHour (int hr){
		Calendar cal = Calendar.getInstance();
